...
def run_two_phase_training(dataset_path: str, output_dir: str = "two_phase_output"):
    ...
    config = PipelineConfig(
        ...
        validation=ValidationConfig(
            frequency="every_epoch",
            metrics=["contrastive_loss", "embedding_quality", "classification_accuracy"],
            save_predictions=True,
            generate_plots=True
        ),
    ...
    try:
        ...
        print(f"📁 Pre-trained model: {results.phase1_results.final_model_path if results.phase1_results else 'N/A'}")
        print(f"📁 Final model: {results.final_model_path}")
        print(f"📁 Best model: {results.best_model_path}")
        print(f"⏱️  Total duration: {results.total_duration:.2f} seconds")
        print(f"📊 Reports generated: {len(results.report_paths)} files")
        ...
        if results.phase1_results:
            print(f"🔹 Phase 1 final loss: {results.phase1_results.final_loss:.4f}")
        if results.phase2_results:
            print(f"🔹 Phase 2 final accuracy: {results.phase2_results.get('final_accuracy', 'N/A')}")
...
def main():
    ...
    parser.add_argument("--dataset", required=True, help="Path to dataset file (JSONL format)")
    ...
    try:
        if args.mode == "single":
            results = run_single_phase_training(args.dataset, f"{args.output_dir}/single_phase")
            
        elif args.mode == "two-phase":
            results = run_two_phase_training(args.dataset, f"{args.output_dir}/two_phase")
            
        elif args.mode == "both":
            print("🔄 Running both training modes for comparison...")
            
            # Run single-phase
            single_results = run_single_phase_training(args.dataset, f"{args.output_dir}/single_phase")
            
            # Run two-phase
            two_phase_results = run_two_phase_training(args.dataset, f"{args.output_dir}/two_phase")
            
            # Compare results
            print("\n📊 Training Comparison:")
            print(f"Single-phase duration: {single_results.total_duration:.2f}s")
            if two_phase_results:
                print(f"Two-phase duration: {two_phase_results.total_duration:.2f}s")
