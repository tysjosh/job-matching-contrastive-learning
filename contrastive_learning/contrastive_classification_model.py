...
class ContrastiveClassificationModel(nn.Module):
    ...
    def __init__(self, config: TrainingConfig):
        ...
        logger.info(f"ContrastiveClassificationModel initialized with embedding_dim={self.embedding_dim}")
        logger.info(f"Contrastive layers frozen: {config.freeze_contrastive_layers}")
    ...
    def _validate_config(self):
        ...
        if not self.config.pretrained_model_path:
            raise ValueError("pretrained_model_path is required for ContrastiveClassificationModel")
        ...
        if not 0.0 <= self.config.classification_dropout <= 1.0:
            raise ValueError(f"classification_dropout must be between 0.0 and 1.0, got: {self.config.classification_dropout}")
    ...
    def _load_pretrained_encoder(self) -> nn.Module:
        ...
        try:
            ...
            logger.info(f"Loading pre-trained contrastive encoder from: {checkpoint_path}")
            ...
            encoder = self._create_contrastive_encoder_from_state_dict(model_state_dict)
    ...
    def _create_contrastive_encoder_from_state_dict(self, state_dict: Dict[str, torch.Tensor]) -> nn.Module:
        ...
        projection_layers = [key for key in state_dict.keys() if 'projection_head' in key]
        
        if not projection_layers:
            raise ValueError("Could not find projection_head layers in pretrained model")
        ...
        if output_dim is None:
            ...
            logger.warning(f"Could not determine output dimension, using config default: {output_dim}")
        ...
        logger.info(f"Created contrastive encoder with input_dim={input_dim}, output_dim={output_dim}")
        return encoder
    ...
    def _freeze_contrastive_layers(self):
        ...
        frozen_params = sum(p.numel() for p in self.contrastive_encoder.parameters())
        trainable_params = sum(p.numel() for p in self.classification_head.parameters() if p.requires_grad)
        
        logger.info(f"Frozen contrastive encoder parameters: {frozen_params:,}")
        logger.info(f"Trainable classification head parameters: {trainable_params:,}")
    ...
    def forward(self, resume_text_emb: torch.Tensor, job_text_emb: torch.Tensor) -> torch.Tensor:
        ...
        resume_emb, job_emb = self._generate_embeddings(resume_text_emb, job_text_emb)
        ...
        return classification_output.squeeze(-1)  # Remove last dimension for binary classification
