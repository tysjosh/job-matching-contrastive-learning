...
class MinimalModel(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        # MiniLM embedding size to projection
...
def test_minimal_training():
    ...
    for batch_idx in range(3):  # Test 3 batches
        ...
        try:
            ...
            anchor_embs = torch.randn(
                batch_size, embedding_dim, requires_grad=False)
            positive_embs = torch.randn(
                batch_size, embedding_dim, requires_grad=False)
            negative_embs = torch.randn(
                batch_size, embedding_dim, requires_grad=False)
